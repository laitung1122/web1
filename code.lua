...
if islclosure(getgenv().setfenv) then
	game.Players.LocalPlayer:Kick("MMB 1")
	while true do
	end
end -- ANTI SETFENV TAMPERING
if islclosure(getgenv().tostring) then
	game.Players.LocalPlayer:Kick("MMB 2")
	while true do
	end
end -- ANTI TOSTRING TAMPERING
...
do
	...
	if islclosure(Random.new) then
		game.Players.LocalPlayer:Kick("MMB 3")
		while true do
		end
	end

	if islclosure(HttpService.JSONDecode) then
		game.Players.LocalPlayer:Kick("MMB 4")
		while true do
		end
	end

	if islclosure(HttpService.JSONEncode) then
		game.Players.LocalPlayer:Kick("MMB 5")
		while true do
		end
	end
	if islclosure(identifyexecutor) then
		game.Players.LocalPlayer:Kick("MMB 6")
		while true do
		end
	end
	if islclosure(REQ) and identifyexecutor() ~= "Delta" and identifyexecutor() ~= "Delta Android" then
		game.Players.LocalPlayer:Kick("MMB 7")
		while true do
		end
	end
...
local function update_seed()
	for i, v in pairs(game:GetService("Players"):GetChildren()) do
		...
		pcall(function()
			RndSeed = (
				RndSeed
				+ mathfloor((v.Head.Position.X * 1000) % 10000 + 2)
					* mathfloor((v.Head.Position.Y * 1000 + 2) % 100000)
			) % 10000000
		end)
	end
end
...
local function base64_encode(data)
	return (
		(data:gsub(".", function(x)
			...
			for i = 8, 1, -1 do
				r = r .. (base64_chars % 2 ^ i - base64_chars % 2 ^ (i - 1) > 0 and "1" or "0")
			end
			return r
...
local function wl(key)
	...
	local response = REQS({
		Url = URL
			...
			.. client_sign
			.. "&e="
			.. base64_encode(rndStr .. "." .. publickeyclient .. "." .. server_time)
			.. "&g="
			.. Encode(client_hwid, publickeyclient),
		Method = "GET",
	})
				...
				["embeds"] = {
					{
						...
						["footer"] = {
							["text"] = "Execute: "
								.. os.date(("%s %s"):format("%x", "%X"), math.floor(server_time / 1000)),
						},
						["fields"] = {
							{
	...
	if sign_match == decode_response.sign then
		...
		function ReadSetting()
			Returner = {}
			Scc, scc2 = pcall(function()
				Returner = HttpService:JSONDecode(
					readfile(Hub.Name .. "/" .. game.Players.LocalPlayer.Name .. "-" .. Hub.Game .. ".json")
				)
			end)
			if
				not Scc
				or not isfolder(Hub.Name)
				or not isfile(Hub.Name .. "/" .. game.Players.LocalPlayer.Name .. "-" .. Hub.Game .. ".json")
			then
				Save()
			end
			Scc, scc2 = pcall(function()
		...
		repeat
			pcall(function()
				task.wait()
				if
					game:GetService("Players").LocalPlayer.PlayerGui:WaitForChild("Main"):FindFirstChild("ChooseTeam")
				then
		...
		function Window:NewTab(mmb)
			...
			function newmm:NewParagraph(cf)
				...
				paragraphnew = domain:AddParagraph({
					Content = newContent,
					Title = newTitle,
				})
			...
			function newmm:NewSlider(scriptitle, cf)
				...
				DFConfig.Callback = cf.Callback or function(v) end
			...
			function newmm:NewDropdown(title, cf)
				...
				dropdown:OnChanged(function(cv)
					pcall(function(cv)
						Save()
					end)
				end)
		...
		function CountZombie(x)
			...
			for i, v in pairs(game.workspace.Enemies:GetChildren()) do
				if RemoveLevelTitle(v.Name) == "Living Zombie" and v.Humanoid.Health > 0 then
					if
						not x
						or (v.HumanoidRootPart.Position - Vector3.new(
								-10171.7607421875,
								138.62667846679688,
								6008.0654296875
							)).magnitude
		...
		function BringMobSoulGuitar()
			pcall(function()
				for i, v in pairs(game.workspace.Enemies:GetChildren()) do
					if
						RemoveLevelTitle(v.Name) == "Living Zombie"
			...
			for i, v in pairs(game.Players:GetChildren()) do
				pcall(function()
					if
						...
						and GetDistance(v.Character.HumanoidRootPart, gggggggggggggg) < min3
					then
		...
		function CheckQuestSoulGuitar()
			if not (Sea2 or Sea3) or CheckItem("Soul Guitar") or LocalPlayerLevelValue < 2300 then
				return
			end
			MaterialReq = {
				Ectoplasm = 250,
			...
			if not game.ReplicatedStorage.Remotes["CommF_"]:InvokeServer("GuitarPuzzleProgress", "Check") then
				if
					(CheckMoon() == "Full Moon" and (game.Lighting.ClockTime > 15 or game.Lighting.ClockTime < 5))
					or Config["HopOption"]
				then
					return "Full Moon"
				end
			...
			if typeof(st) == "string" then
				...
				if st == "Full Moon" then
					if
						...
						and (game.Lighting.ClockTime > 15 or game.Lighting.ClockTime < 5)
					then
						if game.Lighting.ClockTime > 18 or game.Lighting.ClockTime < 5 then
							...
							if GetDistance(CFrame.new(-8654.314453125, 140.9499053955078, 6167.5283203125)) < 10 then
								...
								require(game.ReplicatedStorage.Util.Sound):Play(
									"Thunder",
									workspace.CurrentCamera.CFrame.p
								)
								CreateUiNotify({
									Content = game.ReplicatedStorage.Remotes["CommF_"]:InvokeServer(
										"gravestoneEvent",
										2,
										true
									),
		...
		function FlyBoat(boat, enable)
			...
			boat.VehicleSeat.CFrame =
				CFrame.new(boat.VehicleSeat.CFrame.X, boat.VehicleSeat.CFrame.Y + 250, boat.VehicleSeat.CFrame.Z)
		end
		function getAnyMob()
		...
		task.spawn(function()
			game.workspace.Enemies.ChildAdded:Connect(function(v)
				repeat
					task.wait()
				until v:FindFirstChild("HumanoidRootPart")
				wait(2)
		...
		function CheckPirateRaid()
			for i, v in pairs(game.workspace.Enemies:GetChildren()) do
				if
					v:FindFirstChild("Pirate Spawned Tick")
					and v:FindFirstChild("Humanoid")
					and v.Humanoid.Health > 0
					and v:FindFirstChild("HumanoidRootPart")
				then
		...
		function CheckQuestCDK()
			...
			if CDK_LevelQuest.Good ~= -2 then
				...
				if
					(v318 and v318 > 0)
					or (CheckBoss("Soul Reaper [Lv. 2100] [Raid Boss]") or CheckTool("Hallow Essence"))
				then
					return "Soul Reaper"
				end
			end
			...
			if QQ ~= "None" then
				if QQ == "Final Boss" then
					repeat
						task.wait()
						if
							GetDistance(game:GetService("Workspace").Map.Turtle.Cursed.Pedestal3) > 10
							and game:GetService("Workspace").Map.Turtle.Cursed.PlacedGem.Transparency ~= 0
						then
							Tweento(
								game:GetService("Workspace").Map.Turtle.Cursed.Pedestal3.CFrame * CFrame.new(0, 0, -2)
							)
						end
						if game:GetService("Workspace").Map.Turtle.Cursed.PlacedGem.Transparency == 0 then
							...
							if GetDistance(game:GetService("Workspace").Map.Turtle.Cursed.Pedestal3) < 10 then
								fireproximityprompt(
									game:GetService("Workspace").Map.Turtle.Cursed.Pedestal3.ProximityPrompt
								)
								wait(5)
							end
						end
		...
		function CheckEspHaze()
			...
			for i, v in next, game.workspace.Enemies:GetChildren() do
				if
					RemoveLevelTitle(v.Name) == CheckNearestMobSpawnHaze()
					and v:IsA("Model")
					and v:FindFirstChild("HumanoidRootPart") --and v:FindFirstChild("HazeESP")
					and v:FindFirstChild("Humanoid")
					and v.Humanoid.Health > 0
				then
					xx = v
		...
		function Do_CDK_Yama(st)
			...
			if st == "Yama Dimension" then
				...
				until not Config["Auto CDK"]
					or GetDistance(game:GetService("Workspace")["_WorldOrigin"].Locations["Hell Dimension"]) > 2000
			elseif st == "Easy Quest Yama -3" then
				...
				until not Config["Auto CDK"]
					or not MobHaki
					or not MobHaki.Parent
					or game.Players.LocalPlayer.Character.Humanoid.Health <= 0
					...
					if Target then
						pcall(function()
							...
							until not Config["Auto CDK"]
								or game.Players.LocalPlayer.Character.Humanoid.Health <= 0
								or GetDistance(
										game:GetService("Workspace")["_WorldOrigin"].Locations["Hell Dimension"]
									)
									< 2000
							wait(3)
						end)
		...
		function Do_CDK_Tushita(st)
			...
			if st == "Tushita Dimension" then
				repeat
					task.wait()
					if
						game:GetService("Workspace").Map.HeavenlyDimension.Exit.BrickColor
				...
				until not Config["Auto CDK"]
					or GetDistance(game:GetService("Workspace")["_WorldOrigin"].Locations["Heavenly Dimension"])
						> 2000
		...
		browhat = {
			...
			["Dragon Claw"] = function()
				dragonclawtrue = game.ReplicatedStorage.Remotes.CommF_:InvokeServer(
					"BlackbeardReward",
					"DragonClaw",
					"1"
				) == 1
		...
		function BuyMelee(vcc, oh)
			...
			if oh and LocalPlayerBeliValue and LocalPlayerFragmentValue and not CheckTool(v) then
				...
				if
					pricemelee[v]
					and pricemelee[v].Beli
					and pricemelee[v].Beli <= LocalPlayerBeliValue
					and pricemelee[v].Fragment
					and pricemelee[v].Fragment <= LocalPlayerFragmentValue
				then
					momo = browhat[v]
					if type(momo) == "string" then
						UnequipWeapon()
		...
		function GetFruitInWorkSpace(ConfigF)
			...
			if not valuefruitreq then
				for i, v in pairs(game.Workspace:GetChildren()) do
					if
						string.find(v.Name, "Fruit")
						and v:FindFirstChild("Handle")
						and game.Workspace:FindFirstChild(v.Name)
						and isstored(v)
					then
						return v
					end
		...
		function StoreDFInPath(path)
			for i, v in pairs(path:GetChildren()) do
				if
					string.find(v.Name, "Fruit")
					and not v:FindFirstChild("Coone")
					and not v:FindFirstChild("Raid Client")
				then
					...
					elseif v.Name == "Bird: Falcon Fruit" then
						game:GetService("ReplicatedStorage").Remotes.CommF_
							:InvokeServer("StoreFruit", "Bird-Bird: Falcon", path:FindFirstChild("Bird: Falcon Fruit"))
					else
			...
			if game.Players.LocalPlayer.Character.Humanoid.Sit then
				if GetDistance(BCFrame, GetLocalBoat().VehicleSeat) > 100 then
					...
					repeat
						...
						DoNoClip(true)
						task.wait()
					until GetDistance(BCFrame, GetLocalBoat().VehicleSeat) < 100
						or not game.Players.LocalPlayer.Character.Humanoid.Sit
						or GetDistance(GetLocalBoat().VehicleSeat) > 100
		...
		function GetNMob()
			...
			for i, v in pairs(game.workspace.Enemies:GetChildren()) do
				if
					table.find(NMob, RemoveLevelTitle(v.Name))
					and v:FindFirstChild("Humanoid")
					and v.Humanoid.Health > 0
				then
					return v
				...
				if game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible then
					if GetDistance(TempleCFrame) <= 380 then
						for i, v in pairs(game.Players:GetChildren()) do
							if
								v.Name ~= game.Players.LocalPlayer.Name
								and (GetDistance(TempleCFrame, v.Character.HumanoidRootPart) <= 300)
		...
		function GetSeaBeastTrial()
			...
			if FishmanTrial and GetDistance(FishmanTrial) < 2000 then
				for i, v in next, game:GetService("Workspace").SeaBeasts:GetChildren() do
					if
						string.find(v.Name, "SeaBeast")
						and v:FindFirstChild("HumanoidRootPart")
						and (v.HumanoidRootPart.Position - FishmanTrial.Position).Magnitude <= 1500
					then
		...
		function ReloadChestFunction()
			...
			if
				not game.Workspace:FindFirstChild("Chest1")
				and not game.Workspace:FindFirstChild("Chest2")
				and game.Workspace:FindFirstChild("Chest3")
			then
		...
		function GetALLNPC()
			...
			for i, v in pairs(getnilinstances()) do
				if
					...
					and v.Head.QuestBBG.Title.Text == "MISC."
				then
			...
			for i, v in pairs(game.Players:GetChildren()) do
				pcall(function()
					if
						...
						and v.Data.Level
						and (blacklists and not blacklists[v.Name])
						and v.Character
						and (
		...
		task.spawn(function()
			while task.wait() do
				if Sea2 then
					Nigga = game:GetService("ReplicatedStorage").Remotes.CommF_
						:InvokeServer("BartiloQuestProgress", "Bartilo")
					task.wait(1)
				end
			end
		end)
		function CheckUpgradeRace()
			...
			if Sea2 then
				...
				if Nigga ~= 3 then
					return "Bartilo " .. tostring(Nigga)
				elseif Nigga == 3 then
					if string.find(CheckRace(), "V1") then
						return "Up V2"
					elseif
						LocalPlayerLevelValue >= 1000
						and LocalPlayerBeliValue >= 2000000
						and string.find(CheckRace(), "V2")
						and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("Wenlocktoad", "1") ~= nil
					then
						return "Up V3"
					end
				end
		...
		function CheckQuestBartilo()
			...
			if Nigga == 0 then
				return true
			elseif Nigga == 1 and (CheckBoss("Jeremy [Lv. 850] [Boss]") or Config["Hop Boss"]) then
						...
						pcall(function()
							...
							repeat
								...
								if
									...
									and not game.Players.LocalPlayer.PlayerGui.Main.PvpDisabled.Visible
								then
									LegitAttack = true
									autospamskill = true
							...
							until not ChoDienCanNguoi
								or not getLowestLevelPlayer()
								or not ChoDienCanNguoi
								or not memay2
								or not game.Workspace.Characters:FindFirstChild(getLowestLevelPlayer())
								or not memay2.Character
								or memay2.Character.Humanoid.Health <= 0
								or CheckCantAttackPlayer(memay2)
								or CheckSafeZone(memay2.Character)
								or cc2[getLowestLevelPlayer()]
		...
		function CheckSkillMastery()
			...
			if MasteryWeapon and MasteryWeapon ~= "" then
				if
					game.Players.LocalPlayer.Backpack:FindFirstChild(MasteryWeapon)
					or game.Players.LocalPlayer.Character:FindFirstChild(MasteryWeapon)
				then
					if game:GetService("Players").LocalPlayer.PlayerGui.Main.Skills:FindFirstChild(MasteryWeapon) then
						for i, v in
							next,
							game:GetService("Players").LocalPlayer.PlayerGui.Main.Skills[MasteryWeapon]:GetChildren(
)
						do
							if v:IsA("Frame") then
								if
									Config["Use Skills"][v.Name]
										and v.Name ~= "Template"
										and v.Title.TextColor3 == Color3.new(1, 1, 1)
										and v.Cooldown.Size == UDim2.new(0, 0, 1, -1)
									or v.Cooldown.Size == UDim2.new(1, 0, 1, -1)
								then
		...
		function CanChoosePoint()
			absd, bbbs = pcall(function()
				if
					game.ReplicatedStorage.Remotes.CommF_:InvokeServer("TempleClock", "Check")
					and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("TempleClock", "Check").HadPoint
				then
					return true
				end
			end)
		...
		function InstantChooseGear()
			...
			if v14 and v14.HadPoint then
				...
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame =
					workspace.Map["Temple of Time"].Prompt.CFrame
				if
					(
		...
		task.spawn(function()
			while task.wait() do
				if AimBotSkillPosition then
					...
					PointAimbot:FindFirstChild("AimbotEsp"):FindFirstChild("TextLabel").Text =
						GetDistance(CFrame.new(AimBotSkillPosition.X, AimBotSkillPosition.Y, AimBotSkillPosition.Z))
				end
			end
		end)
		task.spawn(function()
			while task.wait() do
				esp1, esp2 = pcall(function()
		...
		function IsSwanPirate()
			for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
				if
					RemoveLevelTitle(v.Name) == "Swan Pirate"
					and v:FindFirstChild("Humanoid")
					and v.Humanoid.Health > 0
				then
					return true
		...
		function CheckCakePrinceStatus()
			ab, bb = pcall(function()
				...
				if
					CheckBoss("Cake Prince [Lv. 2300] [Raid Boss]") or CheckBoss("Dough King [Lv. 2300] [Raid Boss]")
				then
					...
					return tonumber(
						string.match(
							game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CakePrinceSpawner", true),
							"%d+"
						)
					) .. " Mobs Remaining..."
				end
		...
		function EnableKen()
			if
				...
				and game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel")
			then
			else
				wait(1)
		...
		function checkskillDF()
			if
				not game:GetService("Players").LocalPlayer.PlayerGui.Main.Skills
					:FindFirstChild(game:GetService("Players").LocalPlayer.Data.DevilFruit.Value)
			then
			...
			for i, v in
				next,
				game:GetService("Players").LocalPlayer.PlayerGui.Main.Skills[game:GetService("Players").LocalPlayer.Data.DevilFruit.Value]
					:GetChildren()
			do
		...
		function autoskill()
			...
			if not Config["Use Weapon"] then
				return print("not use wp")
			end
			sword = checkskillSword()
			meele = checkskillMelee()
			df = checkskillDF()
			...
			if
				df
				and Config["Use Weapon"]["Devil Fruit"]
				and not string.find(game:GetService("Players").LocalPlayer.Data.DevilFruit.Value, "Portal")
				and df ~= "F"
			then
				EquipWeaponName(game:GetService("Players").LocalPlayer.Data.DevilFruit.Value)
		...
		function CheckPirateBoat()
			...
			if Config["Auto Ghost Ship"] then
				table.insert(PirateBoats, "FishBoat")
				table.insert(PirateBoats, "PirateGrandBrigade")
			end
				...
				if Config["Auto Sea Beast"] then
					pcall(function()
						if GetDistance(GetLocalBoat().VehicleSeat) < 50 then
							...
							elseif
								not game:GetService("Players").LocalPlayer.Character.Humanoid.Sit
								and tick() - sbtick > 15
							then
			...
			for i, v in next, game:GetService("Workspace").Boats:GetChildren() do
				if v:IsA("Model") then
					if
						...
						and tostring(v.Owner.Value) == game:GetService("Players").LocalPlayer.Name
		...
		function KillBoat(boat)
			...
			repeat
				boat, boat2 = pcall(function()
					...
					getgenv().psskill = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
						* CFrame.new(0, -5, 0)
					if GetDistance(v.Engine.CFrame) < 100 then
						autospamskill = true
					else
						getgenv().psskill = nil
						autospamskill = nil
		...
		function IsStunning()
			...
			if ss2 ~= 0 then
				if
					game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart"):FindFirstChild("BodyVelocity")
				then
					v = game.Players.LocalPlayer.Character
				...
				if Config["Auto Active Race"] then
					pcall(function()
						if
							...
							and not game.Players.LocalPlayer.Character.RaceTransformed.Value
						then
					...
					pcall(function()
						if not game.Players.LocalPlayer.Character:FindFirstChild("Check Mod") then
							...
							for i, v in next, getgc() do
								...
								elseif
									getfenv(v).script == game.Players.LocalPlayer.Character:WaitForChild("Skyjump")
								then
									spawn(function()
								...
								elseif
									getfenv(v).script == game.Players.LocalPlayer.Character:WaitForChild("Dodge")
								then
									for i2, v2 in next, getupvalues(v) do
		...
		spawn(function()
			game:GetService("RunService").Stepped:Connect(function()
				bug, b2 = pcall(function()
					if
						NoClip
						or Config["No Clip"]
							and not getgenv().SittingForSeaBeast
							and tick() - DisableNoClipIn5s >= 5
					then
						DoNoClip(true)
		...
		function GetBossTable()
			...
			for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
				if
					not string.find(v.Name, "1500")
						and (not islongma(v.Name) or (islongma(v.Name) and CheckTushitaGate()))
						and string.find(v.Name, " %pBoss%p")
					or string.find(v.Name, " %pRaid Boss%p")
						and not table.find(blackboss, v.Name)
						and v:FindFirstChild("Humanoid")
						and v.Humanoid.Health > 0
				then
			...
			for i, v in pairs(game.ReplicatedStorage:GetChildren()) do
				if
					not string.find(v.Name, "1500")
						and (v.Name ~= "Longma [Lv. 2000] [Boss]" or (v.Name == "Longma [Lv. 2000] [Boss]") and CheckTushitaGate())
						and string.find(v.Name, " %pBoss%p")
					or string.find(v.Name, " %pRaid Boss%p")
						and not table.find(blackboss, v.Name)
						and v:FindFirstChild("Humanoid")
		...
		function TweenTemple()
			game:GetService("ReplicatedStorage").Remotes.CommF_
				:InvokeServer("requestEntrance", Vector3.new(28282.5703125, 14896.8505859375, 105.1042709350586))
		end
		Mob = ""
		Mob1 = ""
		...
		function CheckMob(tbmob, wijwerg)
			...
			for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
				if
					table.find(tbmob, RemoveLevelTitle(v.Name))
					and v:FindFirstChild("Humanoid")
					and v:FindFirstChild("HumanoidRootPart")
					and v.Humanoid.Health > 0
				then
		...
		function getNextIsland()
			...
			for i, v in next, TableIslandsRaid do
				if
					IsIslandRaid(v)
					and (IsIslandRaid(v).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
						<= 4500
				then
		...
		function CheckSafeZone(p)
			for i, v in pairs(game:GetService("Workspace")["_WorldOrigin"].SafeZones:GetChildren()) do
				if v:IsA("Part") then
					if
						(v.Position - p.HumanoidRootPart.Position).magnitude <= 400
		...
		function GetCFrameADD(v2)
			HealthPersent = Config["Panic Mode"]
				and math.floor(
						(
		...
		function IsIslandRaid(nu)
			if game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island " .. nu) then
				...
				for i, v in pairs(game:GetService("Workspace")["_WorldOrigin"].Locations:GetChildren()) do
					if
						v.Name == "Island " .. nu
						and (v.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
							< min
					then
		...
		function CheckIsRaiding()
			...
			if
				not ALLCHECK_Func["Auto Trial Stage 1"]()
				and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible
				and not GetSeaBeastTrial()
				and GetDistance(CFrame.new(28282.5703125, 14896.8505859375, 105.1042709350586)) > 3000
			then
				return true
			end
		end
		...
		function getCakePrince()
			for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
				if
					table.find(CakePrinceBosses, v.Name)
					and v:FindFirstChild("Humanoid")
					and v:FindFirstChild("HumanoidRootPart")
					and v.Humanoid.Health > 0
				then
		...
		function GetNearestPlayer(pos)
			...
			for i, v in pairs(game.Players:GetChildren()) do
				if
					...
					and (v.Character.HumanoidRootPart.Position - pos).Magnitude < ner
				then
		...
		if not syn then
			function isnetworkowner2(p1)
				...
				if C then
					...
					if
						game.IsDescendantOf(p1, B)
						or (C.Position - p1.Position).Magnitude <= A and GetNearestPlayer(p1.Position)
					then
						return true
					end
				end
		...
		function BringMob(BringMobChoosen, V5)
			...
			if BringMobChoosen then
				for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
					if
						v.Name == BringMobChoosen.Name
						and (V6.Position - v.HumanoidRootPart.Position).Magnitude < 340
						and (chodonandngu(v.HumanoidRootPart) or isnetworkowner(v.HumanoidRootPart))
						and v.Humanoid.MaxHealth < 100000
					then
						spawn(function()
				...
				for i, v in pairs(game.Players:GetChildren()) do
					pcall(function()
						...
						if GetDistance(v.Character.HumanoidRootPart, CF) < Config["Hop Near Distance"] then
							CreateUiNotify({
								Content = "Player: " .. tostring(v.Name) .. " reached distance: " .. tostring(
									math.floor(GetDistance(v.Character.HumanoidRootPart))
								),
							})
							HopServer(9, true)
						end
		...
		function KillNigga(Nasga)
			if
				Nasga
				and Nasga:FindFirstChild("Humanoid")
				and Nasga:FindFirstChild("HumanoidRootPart")
				and Nasga.Humanoid.Health > 0
			then
				...
				repeat
					...
					then
						...
						spawn(function()
							bbc, bbc2 = pcall(function()
								MMBStatus = "Killing "
									.. Nasga.Name
									.. " | Health Remaining: "
									.. math.floor(Nasga.Humanoid.Health)
								if Nasga:FindFirstChild("TickCheck") then
									MMBStatus = MMBStatus
										.. " | Delete Nasga In: "
										.. math.floor(tonumber(90 - (tick() - Nasga:FindFirstChild("TickCheck").Value)))
										.. "s"
								end
							end)
						end)
						...
						CanMasteryFarm = Nasga.Humanoid.Health
							<= (Nasga.Humanoid.MaxHealth * Config["%HealthSendSkill"]) / 100
						if
							Config["Mastery Farm"]
							and CheckOnlyV4()
							and Config["Mastery Farm"]
							and Nasga.Humanoid.MaxHealth < 500000
							and CanMasteryFarm
						then
							if
								Config["Mastery Farm"]
							...
							then
								...
								until not Nasga
									or not Nasga.Parent
									or not Nasga:FindFirstChild("Humanoid")
									or Nasga.Humanoid.Health <= 0
									or not CheckOnlyV4()
									or not Config["Mastery Farm"]
									or Nasga.Humanoid.Health
										> (Nasga.Humanoid.MaxHealth * Config["%HealthSendSkill"]) / 100
								UseFastAttack = false
								NoClip = false
				...
				until not Nasga
					or not Nasga:FindFirstChild("Humanoid")
					or not Nasga:FindFirstChild("HumanoidRootPart")
					or Nasga.Humanoid.Health <= 0
					or CheckIsRaiding()
				pcall(function()
		...
		function GetMidPoint(MobName, b2)
			...
			for i, v in pairs(game.workspace.Enemies:GetChildren()) do
				if
					...
					and v:FindFirstChild("Humanoid")
					and v:FindFirstChild("HumanoidRootPart")
					and (b2 and GetDistance(v.HumanoidRootPart, b2) <= 475)
				then
		...
		function Tweento(targetCFrame)
			bbc11, bbc12 = pcall(function()
				...
				then
					...
					oldcframe = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
					Distance = (targetPos - game:GetService("Players").LocalPlayer.Character
						:WaitForChild("HumanoidRootPart").Position).Magnitude
					...
					if Config["Panic Mode"] and IsLowHealth then
						...
						until not Config["Panic Mode"]
							or not game.Players.LocalPlayer.Character:FindFirstChild("Humanoid")
							or game.Players.LocalPlayer.Character.Humanoid.Health > NotLowHealth
						AntiLowHealth(OldY)
					...
					if Config["Same Y"] and (b1.Position - targetCFrameWithDefualtY.Position).Magnitude > 5 then
						...
						local info = TweenInfo.new(
							(targetPos - game:GetService("Players").LocalPlayer.Character
								:WaitForChild("HumanoidRootPart").Position).Magnitude
								/ TweenSpeed,
		...
		local function CheckQuest()
			...
			elseif
				lvlPl >= 2075
				and autofullymelee
				and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyDragonTalon", true) ~= 1
				and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyElectricClaw", true) == 1
			then
				Mob1 = "Posessed Mummy"
				Mob2 = "HauntedQuest2"
				Mob3 = 2
					...
					for i1, v1 in pairs(v) do
						...
						for i2, v2 in pairs(v1.Task) do
							if
								...
								and lvlreq >= min
								and v1.Task[i2] > 1
								and not table.find(UselessQuest, tostring(i))
							then
								min = lvlreq
								Mob1 = tostring(i2)
								Mob2 = i
		...
		function CFrameQuest()
			...
			for i, v in pairs(getnilinstances()) do
				if
					...
					and v:FindFirstChild("Head")
					and v.Head:FindFirstChild("QuestBBG")
					and v.Head.QuestBBG.Title.Text == "QUEST"
				then
			...
			QuestPoint["SkyExp1Quest"] = CFrame.new(
				-7857.28516,
				5544.34033,
				-382.321503,
				-0.422592998,
					...
					for i2, v2 in pairs(v1.Task) do
						if i2 == Mob1 then
							for i3, v3 in next, v do
								if
									v3.LevelReq <= game.Players.LocalPlayer.Data.Level.Value
									and v3.Name ~= "Town Raid"
								then
		...
		function CheckDoubleQuest2()
			...
			if
				...
				and Config["Double Quest"]
				and CheckQuestData()
				and CheckNameDoubleQuest() == Mob1
				and #CheckNameDoubleQuest() > 2
			then
				for i, v in pairs(Quest) do
					for i1, v1 in pairs(v) do
						for i2, v2 in pairs(v1.Task) do
		...
		function MobLevel1OrMobLevel2()
			...
			for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
				if
					not table.find(mbb, v.Name)
					and v:IsA("Model")
					and v.Name ~= "PirateBasic"
					and not string.find(v.Name, "Brigade")
					and v:FindFirstChild("Humanoid")
					and v.Humanoid.Health > 0
					and v:FindFirstChild("HumanoidRootPart")
				then
		...
		function MobLevelSkip()
			for i, v in next, game.workspace.Enemies:GetChildren() do
				if
					v:IsA("Model")
					and table.find(MBLVSKIP, RemoveLevelTitle(v.Name))
					and v:FindFirstChild("HumanoidRootPart")
					and v.Humanoid.Health > 0
				then
					return v.Name
		...
		function HopLow()
			...
			if
				SiteHopServerLess.nextPageCursor
				and SiteHopServerLess.nextPageCursor ~= "null"
				and SiteHopServerLess.nextPageCursor ~= nil
			then
			...
			for i, v in pairs(SiteHopServerLess.data) do
				if v.playing and tonumber(v.playing) <= 4 and v.id ~= game.JobId then
					game:GetService("TeleportService")
						:TeleportToPlaceInstance(game.PlaceId, tostring(v.id), game.Players.LocalPlayer)
				end
			end
		end
					...
					for k, v in pairs(huhu) do
						if k ~= game.JobId and v["Count"] < CountTarget then
							if not Settings2[k] or tick() - Settings2[k].Time > 60 * 10 then
								...
								SaveSettings2()
								if
									game:GetService("Players").LocalPlayer.PlayerGui.Main.InCombat.Bottom.Visible
									and game:GetService("Players").LocalPlayer.PlayerGui.Main.InCombat.Visible
								then
									...
									until not game:GetService("Players").LocalPlayer
										or (
											not game:GetService("Players").LocalPlayer.PlayerGui.Main.InCombat.Bottom.Visible
											and not game:GetService("Players").LocalPlayer.PlayerGui.Main.InCombat.Visible
										)
									AntiLowHealthting = false
									Notify(
										"Script Status",
		...
		task.spawn(function()
			while task.wait() do
				...
				if Config["Auto Raid"] and not Sea1 then
					if
						CheckIsRaiding()
						or game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == true
						or getNextIsland()
					then
						repeat
							wait()
					...
					elseif not CheckTool("Special Microchip") then
						if not GetFruitInCharacter(true) then
							BoughtChip = game.ReplicatedStorage.Remotes.CommF_:InvokeServer(
								"RaidsNpc",
								"Select",
						...
						if BoughtChip then
							...
							repeat
								task.wait()
							until tick() - raidtick >= 60
								or not Config["Auto Raid"]
								or (
									CheckIsRaiding()
									or game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == true
									or getNextIsland()
						...
						elseif not BoughtChip then
							pcall(function()
								if
									Config["Auto Get Fruit Below 1M To Raid"]
									and not GetFruitInCharacter()
									and Config["Raid Selected"] ~= "Dough"
								then
		...
		ALLCHECK_Func = {
			...
			["Auto Yama"] = function()
				if not yamagate then
					yamagate = CheckItem("Yama")
				end
				if yamagate then
					return false
				end
			...
			["Auto Tushita"] = function()
				...
				elseif not CheckTushitaGate() then
					if CheckBoss("rip_indra True Form [Lv. 5000] [Raid Boss]") then
						return true
					elseif
						Config["Auto Tushita Hop"]
						and (not Config["Auto Elite"] or (Config["Auto Elite"] and not CheckElite()))
					then
						return true
					end
				end
			...
			["Auto Trial Stage 1"] = function()
				...
				elseif
					GetSeaBeastTrial()
					and GetDistance(GetSeaBeastTrial().HumanoidRootPart) < 2000
					and GetDistance(
							game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Trial of Water")
						)
						< 200
				then
					return true
			...
			["Farm Normal"] = function()
				if Config["Farm Normal"] and Config["Farm Type"] then
					...
					if
						Config["Farm Type"]["Level"]
						and (CountType <= 1 or (CountType >= 2 and LocalPlayerLevelValue < 2450))
					then
						return true
			...
			["Auto Mirage Puzzle"] = function()
				...
				if Valk and MirrorFractal then
					...
					if v213 == 1 or v213 == 2 or v213 == 3 then
						return true
					elseif
						IsMirageIsland() and not game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CheckTempleDoor")
					then
						return true
					end
				else
		...
		ALL_FUNC = {
			...
			["Auto Raid"] = function()
				if CheckTool("Special Microchip") and not CheckIsRaiding() and not getNextIsland() then
					...
					until not game.Players.LocalPlayer
						or not Config["Auto Raid"]
						or game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible
			...
			["Kill Trial Players"] = function()
				if GetPlayerKillTrial() then
					...
					repeat
						...
						pcall(function()
							...
							if GetDistance(GetPlayerKillTrial().Character.HumanoidRootPart) < 300 then
								game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = GetPlayerKillTrial().Character.HumanoidRootPart.CFrame
									* CFrame.new(0, 5, 3)
								UseFastAttack = true
					...
					until not Config["Kill Trial Players"]
						or (not GetPlayerKillTrial() or v.Name ~= GetPlayerKillTrial().Name)
						or not game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible
						or not v
						or not v.Parent
			...
			["Auto Trial Stage 1"] = function()
				...
				then
					...
					until not Config["Auto Trial Stage 1"]
						or GetDistance(
							game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Trial of Water")
						) > 1500
						or not GetSeaBeastTrial()
						or GetDistance(GetSeaBeastTrial().HumanoidRootPart) > 2000
					...
					then
						if getBlueGear() then
							...
							if BlueGear and not BlueGear.CanCollide and BlueGear.Transparency ~= 1 then
								...
								until not getBlueGear()
									or getBlueGear().Transparency == 1
									or game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CheckTempleDoor")
							elseif BlueGear and BlueGear.Transparency == 1 then
								if
									HighestPointRealCFrame
									and (
											game.Players.LocalPlayer.Character.HumanoidRootPart.Position
											- HighestPointRealCFrame.Position
										).Magnitude
										> 10
								then
						...
						elseif not getBlueGear() and getHighestPoint() then
							if game.Lighting.ClockTime < 18 and game.Lighting.ClockTime > 5 then
								...
								CreateUiNotify({
									Content = "Script Notify",
									"Watting (" .. TimetoNight .. ") Minutes for night!",
								})
							end
					...
					if nig and nig ~= "Has V3/V4" then
						...
						if statusupgraderace == "Bartilo 0" then
							if
								...
								and game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == true
							then
									...
									for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
										...
										then
											pcall(function()
												...
												until not v.Parent
													or v.Humanoid.Health <= 0
													or game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible
														== false
												UseFastAttack = false
											end)
										end
						...
						elseif statusupgraderace == "Bartilo 1" then
							...
							if Jeremy then
								...
								if
									game.Workspace.Enemies:FindFirstChild("Jeremy [Lv. 850] [Boss]")
						...
						elseif statusupgraderace == "Up V2" then
							if
								game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 1")
								and game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 2")
								and game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 3")
							then
								...
								if
									not game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 1")
									and not game.Players.LocalPlayer.Character:FindFirstChild("Flower 1")
								then
								...
								elseif
									not game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 2")
									and not game.Players.LocalPlayer.Character:FindFirstChild("Flower 2")
								then
								...
								elseif
									not game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 3")
									and not game.Players.LocalPlayer.Character:FindFirstChild("Flower 3")
								then
											...
											then
												pcall(function()
													...
													until not AutoUpgradeRace
														or not v.Parent
														or v.Humanoid.Health <= 0
														or game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 3")
														or game.Players.LocalPlayer.Character:FindFirstChild("Flower 3")
													UseFastAttack = false
												end)
											end
										end
						...
						elseif statusupgraderace == "Up V3" then
							...
							if Race == "Skypiea V2" then
								...
								for i, v in pairs(game.Players:GetChildren()) do
									if v then
										if
											v:FindFirstChild("Data")
											and v.Data:FindFirstChild("Race")
											and v.Data.Race.Value == "Skypiea"
											and v.Name ~= game.Players.LocalPlayer.Name
											and not table.find(blacklistskypiea, v.Name)
										then
								...
								if #allplayersky > 0 then
									for i, v in pairs(allplayersky) do
										skypcall, skypcall2 = pcall(function()
											...
											until CheckRace() ~= "Skypiea V2"
												or not CheckKillPlayer(v)
												or game.Players:FindFirstChild(v).Character.Humanoid.Health <= 0
												or not Config["Auto Upgrade Race"]
												or CheckSafeZone(vm.Character)
												or CheckCantAttackPlayer(vm)
											print("return ", v)
										...
										if not skypcall then
											print("skypcall2", skypcall2)
										end
									end
								end
							elseif Race == "Fishman V2" then
								...
								until not Config["Auto Upgrade Race"]
									or not Racemmmb == "Fishman V2"
									or not string.find(Racemmmb, "Fish")
							...
							elseif Race == "Ghoul V2" then
								...
								repeat
									...
									wait(1)
								until not Config["Auto Upgrade Race"]
									or game.Players.LocalPlayer.Data.Race.Value ~= "Ghoul"
									or string.find(CheckRace(), "V3")
								ChoDienCanNguoi = false
									...
									for name, is in pairs(BossSpawned) do
										...
										if is and CheckRace() == "Human V2" then
											...
											repeat
												...
												if
													Target
													and game.Workspace.Enemies:FindFirstChild(Target.Name)
													and Target:FindFirstChild("Humanoid")
			...
			["Boss Farm"] = function()
				if Config["Selected Boss"] then
					CountBoss = 0
					if not getNearestSelectedBoss() then
						return
					end
					Bosslon = {
					...
					for i, v in pairs(Bosslon) do
						if CheckMob({ i }, true) and v then
							if CheckMob({ i }, true) then
									...
									if Config["Get Boss Quest"] then
										if CheckQuestBoss(i) and GetInfoBossQuest(i) then
											...
											if
												...
												or not game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible
											then
								...
								until not Config["Selected Boss"][i]
									or not CheckMob({ i }, true)
									or not Config["Boss Farm"]
								return
							end
						end
					end
				...
				if Config["Farm Normal"] and Config["Farm Type"] then
					...
					then
						if
							...
							and not getgenv().QuestKillPlayer
							and LocalPlayerLevelValue >= 35
							and Config["Kill Player Quest"]
						then
							game.ReplicatedStorage.Remotes["CommF_"]:InvokeServer("PlayerHunter")
							local namequest = string.gsub(
							...
							if
								...
								and namequest
								and game:GetService("Workspace").Characters:FindFirstChild(namequest)
							then
							...
							then
								...
								if
									game:GetService("Players").LocalPlayer.PlayerGui.Main
										:FindFirstChild("Quest").Visible
									and not game:GetService("Workspace").Characters:FindFirstChild(namequest)
								then
									game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
								end
								if
									game.Players[namequest].Data.Level.Value < 20
									or game.Players[namequest].Data.Level.Value > 350
								then
									...
									for i, v in next, game:GetService("Workspace").Characters:GetChildren() do
										if v.Name == namequest then
											...
											repeat
												...
												if game.Players.LocalPlayer.PlayerGui.Main.PvpDisabled.Visible then
													game:GetService("ReplicatedStorage").Remotes.CommF_
														:InvokeServer("EnablePvp")
												end
												spawn(function()
													EquipWeapon()
													EnableBuso()
													if
														getgenv().clickattackmethodfarm
														and (v.HumanoidRootPart.Position - game:GetService(
																"Players"
															).LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
															< 50
													then
														local loa = getupvalues(
															require(
												...
												spawn(function()
													if
														not getgenv().clickattackmethodfarm
														and (v.HumanoidRootPart.Position - game:GetService(
																"Players"
															).LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
															< 150
													then
														...
														repeat
															wait()
														until tick() - condimemeay >= 5
															or (v.HumanoidRootPart.Position - game:GetService(
																	"Players"
																).LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
																>= 300
											...
											until not v
												or not v.Parent
												or v.Humanoid.Health <= 0
												or not Config["Farm Normal"]
												or not game:GetService("Players").LocalPlayer.PlayerGui.Main
													:FindFirstChild("Quest").Visible
												or CheckSafeZone(game.Players[v.Name].Character)
												or (
													GetDistance(game.Players[v.Name].Character.HumanoidRootPart)
														< 150
													and game.Players.LocalPlayer
														:WaitForChild("PlayerGui")
														:WaitForChild("Main")
														:WaitForChild("SafeZone").Visible
												)
											getgenv().clickattackmethodfarm = false
											UseFastAttack = false
											AutoTp = false
						...
						if
							not getgenv().QuestKillPlayer
							or (not Config["Kill Player Quest"] and getgenv().QuestKillPlayer)
						then
							AutoTp = false
							if
								game.PlaceId == 2753915549
								and game.Players.LocalPlayer.Data.Level.Value < 120
								and game.Players.LocalPlayer.Data.Level.Value >= 10
							then
								if not MobLevelSkip() then
									...
									for i, v in pairs(game.workspace.Enemies:GetChildren()) do
										if
											Config["Farm Normal"]
											and Config["Farm Type"]["Level"]
											and v.Name == MobLevelSkip()
											and v:FindFirstChild("HumanoidRootPart")
											and v:FindFirstChild("Humanoid")
											and v.Humanoid.Health > 0
										then
											repeat
												KillMob(v)
											until not Config["Farm Normal"]
												or not Config["Farm Type"]["Level"]
												or not v
												or not v:FindFirstChild("Humanoid")
												or not v:FindFirstChild("HumanoidRootPart")
												or v.Humanoid.Health <= 0
							...
							elseif game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible then
								if not MobLevel1OrMobLevel2() then
									...
									for i, v in pairs(game.workspace.Enemies:GetChildren()) do
										if
											v.Name == MobLevel1OrMobLevel2()
											and v:FindFirstChild("HumanoidRootPart")
											and v:FindFirstChild("Humanoid")
											and v.Humanoid.Health > 0
											and game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible
										then
											repeat
												KillMob(v)
											until not Config["Farm Normal"]
												...
												or not v:FindFirstChild("HumanoidRootPart")
												or v.Humanoid.Health <= 0
												or not game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible
											StartBringMob = false
											UseFastAttack = false
											UseFastAttack = false
											NoClip = false
					...
					elseif Config["Farm Type"]["Cake Prince"] and Sea3 then
						if
							LocalPlayerLevelValue >= 2200
							and Config["Claim Bone & Cake Prince Quest"]
							and not game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible
						then
							levelcustom = 2200
						...
						elseif
							not Config["Claim Bone & Cake Prince Quest"]
							or (
								(Config["Claim Bone & Cake Prince Quest"] and LocalPlayerLevelValue < 2200)
								or game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible
							)
						then
								...
								if Target then
									if game.Workspace.Enemies:FindFirstChild(Target.Name) then
										repeat
											...
											KillMob(Target)
										until not Target
											or not Config["Farm Type"]["Cake Prince"]
											or not Config["Farm Normal"]
											or not getCakePrince()
										UseFastAttack = false
									else
										EnableBuso()
										EquipWeapon()
										if
											GetDistance(
												CFrame.new(-1990.672607421875, 4532.99951171875, -14973.6748046875)
											) > 3000
										then
											Tweento(
												CFrame.new(
		...
		do
			...
			DefaultTab:NewToggle("Same Y", {
				Title = "Same Y Tween",
			})
			DefaultTab:NewToggle("Smart Bring", {
				Title = "Smart Bring",
			})
			DefaultTab:NewDropdown("Wolrd Selected", {
			...
			table.sort(mommb)
			DefaultTab:NewDropdown("Mob Selected", {
				Title = "Choose Mob",
				Values = mommb,
				Callback = function(v)
			...
			DefaultTab:NewDropdown("Stats Table", {
			...
			FarmTab:NewDropdown("Fast Attack Delay", {
			...
			FarmTab:NewToggle("Fast Attack Aura", {
				Title = "Attack Aura",
			})
			FarmTab:AddSection("Default Farm")
			SelectWeapon_Dropdown = FarmTab:NewDropdown("Weapon Type", {
			...
			SelectTypeFarm_Dropdown = FarmTab:NewDropdown("Farm Type", {
			...
			Farm_Toggle = FarmTab:NewToggle("Farm Normal", {
				Title = "Farm",
			})
			DoubleQuest_Toggle = FarmTab:NewToggle("Double Quest", {
				Title = "Double Quest",
				Description = "Double Quest | Recommend for Sea 2/3",
			})
			if Sea1 then
				FarmTab:NewToggle("Kill Player Quest", {
					Title = "Kill Player Quest",
					Description = "Claim Kill Player Quest For Super-High Exp",
				})
			elseif Sea3 then
				ClaimExtraQuest_Toggle = FarmTab:NewToggle("Claim Bone & Cake Prince Quest", {
			...
			SelectBoss_Dropdown = FarmTab:NewDropdown("Selected Boss", {
				Title = "Select Boss",
				Values = getBossSeaHub(),
				Multi = true,
			})
			FarmTab:NewToggle("Get Boss Quest", {
				Title = "Get Boss Quest",
			})
			FarmTab:NewToggle("Boss Farm", {
				Title = "Farm Bosses Selected",
			})
			FarmTab:NewToggle("Hop Boss", {
					...
					while task.wait() do
						svstats, svstats2 = pcall(function()
							...
							SV3 = "Player In Server: "
								...
								.. function8()
								.. "\nAcient One Status: "
								.. tostring(CheckAcientOneStatus())
								.. "\nCake Prince Status: "
								.. tostring(CheckCakePrinceStatus())
								.. "\nMirage Puzzle: "
								.. tostring(CheckGatCan())
								.. "\nMirage Island: "
								.. tostring(CheckMirageIslandStatus())
								.. "\nElite: "
								.. CheckEliteStatus()
								.. "\nRace: "
								.. tostring(CheckRace())
				...
				if Sea3 then
					...
					task.spawn(function()
						while task.wait(1) do
							if
								string.find(CheckAcientOneStatus(), "train")
								or string.find(CheckAcientOneStatus(), "Train")
							then
		...
		loadstring(
			game:HttpGet("https://raw.githubusercontent.com/memaybeohub/Function-Scripts/main/FastAttackLoader.lua")
		)()
		-- Final Load --
		LoadedUiHub = true
		Window:SelectTab(1)
